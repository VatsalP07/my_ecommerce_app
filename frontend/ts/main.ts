...
socket.on('serverMessage', (data: { text: string }) => {
    console.log('[Socket.IO Client]: Received serverMessage:', data.text);
});

socket.on('newOrderCreated', (orderData: { message: string, productName?: string, orderId?: string }) => {
    console.log('[Socket.IO Client]: New order created (pre-payment) event:', orderData);
});

socket.on('stockUpdate', (data: { productId: string, newStock: number }) => {
    console.log('[Socket.IO Client]: Stock update event:', data);
    const event = new CustomEvent('stockUpdatedOnPage', { detail: data });
    document.dispatchEvent(event);
});

socket.on('orderPaymentSuccess', (data: { orderId: string, message: string, userId: string }) => {
...
if (chatSendButton && chatInput) {
    ...
    chatInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendLogic(); }
    });
}
socket.on('chatMessage', (message: { sender: string, text: string }) => {
    if (message.sender === 'You' && document.querySelector('#chat-messages div:last-child')?.textContent?.includes(message.text)) return;
    if (chatWidgetContainer && chatWidgetContainer.style.display !== 'none') {
      displayMessageInChat(message);
    }
});
function displayMessageInChat(message: { sender: string, text: string }) {
    if (chatMessagesDiv) {
...
// --- UI Helper Functions ---
function displayTemporaryNotification(message: string, duration: number = 7000) {
...
function createNotificationArea(): HTMLElement {
    ...
    if (!area) {
        ...
        area.style.position = 'fixed'; area.style.bottom = '20px'; area.style.right = '20px';
        area.style.zIndex = '10000'; area.style.display = 'flex'; area.style.flexDirection = 'column-reverse';
...
export function setToken(receivedTokenValue: string): void {
    if (typeof receivedTokenValue === 'string' && receivedTokenValue.trim() !== '') {
        ...
        if (tokenToStore === '') { console.error('[DEBUG Main - setToken] Token empty after strip.'); return; }
        localStorage.setItem('authToken', tokenToStore);
    } else { console.error('[DEBUG Main - setToken] Attempted to set invalid token.'); }
}
export function removeToken(): void {
    console.log('[DEBUG Main - removeToken] Token removed.');
    localStorage.removeItem('authToken');
}
function getTokenPayload(): { sub: string, email: string, roles: string[], [key: string]: any } | null {
    const token = getToken();
    ...
    } catch (e: any) { console.error("[DEBUG Main - getTokenPayload] Error decoding token:", e.message); return null; }
}
function updateNavUI(token: string | null): void {
    ...
    authLinksContainers.forEach(navConfig => {
        ...
        if (container) {
            ...
            if (token) {
                const logoutLink = document.getElementById(navConfig.logoutButtonId) as HTMLAnchorElement | null;
                if (logoutLink) {
                    const newLogoutLink = logoutLink.cloneNode(true) as HTMLAnchorElement;
                    logoutLink.parentNode?.replaceChild(newLogoutLink, logoutLink);
                    newLogoutLink.addEventListener('click', (e) => {
                        e.preventDefault(); removeToken(); alert('Logged out!'); updateNavAndCart(); window.location.href = navConfig.defaultRedirect;
                    });
                }
            }
        }
...
export async function updateCartCount(): Promise<void> {
    ...
    if (cartCountSpans.length === 0) return;
    const token = getToken();
    if (!token) { cartCountSpans.forEach(span => span.textContent = '0'); return; }
    try {
        const response = await fetch(`${API_BASE_URL}/cart`, { headers: { 'Authorization': `Bearer ${token}` } });
        if (!response.ok) {
            if (response.status === 401) { console.warn('[DEBUG CartCount]: 401 from /cart.'); removeToken(); updateNavUI(null); }
            else { console.error('[DEBUG CartCount]: Failed status:', response.status); }
            cartCountSpans.forEach(span => span.textContent = '0'); return;
        }
        const cartData = await response.json();
        const count = cartData.data?.totalQuantity?.toString() || '0';
        cartCountSpans.forEach(span => span.textContent = count);
    } catch (error) { console.error('[DEBUG CartCount]: Error:', error); cartCountSpans.forEach(span => span.textContent = '0'); }
}
export async function updateNavAndCart(): Promise<void> {
...
if (categoriesBtn && categoriesDropdown) {
    categoriesBtn.addEventListener('click', (event) => {
        event.stopPropagation(); categoriesDropdown.classList.toggle('hidden');
        categoriesBtn.setAttribute('aria-expanded', categoriesDropdown.classList.contains('hidden') ? 'false' : 'true');
    });
    document.addEventListener('click', (event) => {
        if (!categoriesDropdown.classList.contains('hidden') && !categoriesBtn.contains(event.target as Node) && !categoriesDropdown.contains(event.target as Node)) {
            categoriesDropdown.classList.add('hidden'); categoriesBtn.setAttribute('aria-expanded', 'false');
        }
    });
...
// ************************************************************************** //
const navSearchInput = document.getElementById('nav-search-input') as HTMLInputElement | null;
const navSearchButton = document.getElementById('nav-search-btn') as HTMLButtonElement | null;

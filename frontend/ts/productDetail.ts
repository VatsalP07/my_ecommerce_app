...
async function handleAddToCart(productId: string | undefined, quantity: number = 1) { // Added quantity parameter
    if (!productId) {
...
function renderProductDetail(product: any) {
    if (!productDetailContainer || !product) {
        if(productDetailContainer) productDetailContainer.innerHTML = '<p class="text-center text-red-500">Could not load product details.</p>';
        return;
    }
    ...
    if (product.imageKeys && product.imageKeys.length > 1) {
        thumbnailsHTML = product.imageKeys.map((imgUrl: string, index: number) => `
...
function updateStockOnDetailPage(productIdFromEvent: string, newStock: number) {
    ...
    if (currentProductId === productIdFromEvent) {
        ...
        const addToCartBtn = document.getElementById('add-to-cart-btn') as HTMLButtonElement | null; // For re-finding if recreated
        ...
        if (purchaseActionsDiv) {
            if (newStock > 0) {
                ...
                if (!document.getElementById('add-to-cart-btn')) {
                    ...
                    const newAddToCartBtn = document.getElementById('add-to-cart-btn') as HTMLButtonElement | null;
                    if (newAddToCartBtn) {
                    ...
                    const quantityInput = document.getElementById('quantity-input') as HTMLInputElement | null;
                    if (quantityInput) quantityInput.max = newStock.toString();
                    const currentAddToCartBtn = document.getElementById('add-to-cart-btn') as HTMLButtonElement | null;
                    if (currentAddToCartBtn) currentAddToCartBtn.disabled = false;

...
async function fetchCart() {
    ...
    if (!token) {
        ...
        totalItemsSpan.textContent = '0'; totalPriceSpan.textContent = '0.00';
        ...
        if (placeOrderTestBtn) (placeOrderTestBtn as HTMLButtonElement).disabled = true;
        return;
    }
...
function renderCart(cart: any) {
    ...
    if(placeOrderTestBtn) (placeOrderTestBtn as HTMLButtonElement).disabled = false;
...
async function handleQuantityChange(event: Event) {
    ...
    if (isNaN(newQuantity) || newQuantity < 1) { alert('Quantity must be at least 1.'); fetchCart(); return; }
    if (newQuantity > maxStock) { alert(`Cannot exceed available stock (${maxStock}).`); input.value = maxStock.toString(); return; }
    if (!cartItemId) { console.error("Cart item ID not found for quantity change."); return; }
    try {
        ...
        renderCart(result.data);
        updateNavAndCart();
    } catch (error: any) { console.error('Update quantity error:', error); alert(`Error: ${error.message}`); fetchCart(); }
}
...
async function handleRemoveItem(event: Event) {
    ...
    const cartItemId = cartItemDiv?.dataset.cartItemId; // Added optional chaining
    if (!cartItemId || !confirm('Are you sure you want to remove this item?')) return;
    try {
        const response = await fetch(`${API_BASE_URL}/cart/items/${cartItemId}`, {
        ...
        renderCart(result.data);
        updateNavAndCart();
    } catch (error: any) { console.error('Remove item error:', error); alert(`Error: ${error.message}`); fetchCart(); }
}
...
if (clearCartButton) {
    clearCartButton.addEventListener('click', async () => {
        ...
        try {
            ...
            renderCart(result.data);
            updateNavAndCart();
        } catch (error: any) { console.error('Clear cart error:', error); alert(`Error: ${error.message}`); }
    });
}
    ...
    placeOrderTestBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (paymentSuccess) {
                ...
                if (checkoutMessageEl) checkoutMessageEl.textContent = 'Simulated payment failed. Your order was created but payment is pending.';

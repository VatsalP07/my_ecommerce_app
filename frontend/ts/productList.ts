...
const productListHeadingElement = document.getElementById('product-list-heading') as HTMLHeadingElement | null;
const heroSection = document.querySelector('header.bg-gradient-to-r') as HTMLElement | null; // Assuming this is your hero section
...
function animateHeroElements() {
    ...
    if (!headline) return; // Only animate if hero elements are present (i.e., on homepage)

    const subheadline = document.getElementById('hero-subheadline');
    const ctaButton = document.getElementById('hero-cta');

    const chars = headline.querySelectorAll<HTMLElement>('.hero-char');
    chars.forEach((char, index) => {
        setTimeout(() => { char.classList.add('visible'); }, index * 70);
    });
    const headlineAnimationTime = (chars.length * 70) + 600;
    if (subheadline) { setTimeout(() => { subheadline.classList.add('visible'); }, headlineAnimationTime); }
    if (ctaButton) { setTimeout(() => { ctaButton.classList.add('visible'); }, headlineAnimationTime + 300); }
}
...
async function fetchProducts(page: number = 1) {
    ...
    if (searchTerm) {
        ...
        if (shopByCategorySection) (shopByCategorySection as HTMLElement).style.display = 'none';
        ...
        if (shopByCategorySection) (shopByCategorySection as HTMLElement).style.display = 'block'; // Or flex etc.
        if (document.getElementById('hero-headline')) animateHeroElements(); // Animate hero only on default load
    }
...
function renderPaginationControls(pagination: any) {
    ...
    const createButton = (text: string | number, pageNum: number, isCurrent: boolean = false, isDisabled: boolean = false, isEllipsis: boolean = false) => {
        ...
        if (!isDisabled && !isCurrent && !isEllipsis) {
            button.addEventListener('click', () => {
                currentPage = pageNum; fetchProducts(currentPage);
    ...
    if (currentPageNum > pageBuffer + 2 && totalPages > maxPagesToShowOverall) paginationControlsContainer.appendChild(createButton('...', 0, false, true, true));
    let startPage = Math.max(2, currentPageNum - pageBuffer); let endPage = Math.min(totalPages - 1, currentPageNum + pageBuffer);
    for (let i = startPage; i <= endPage; i++) paginationControlsContainer.appendChild(createButton(i, i, currentPageNum === i));
    if (currentPageNum < totalPages - pageBuffer - 1 && totalPages > maxPagesToShowOverall) paginationControlsContainer.appendChild(createButton('...', 0, false, true, true));
    if (totalPages > 1) paginationControlsContainer.appendChild(createButton(totalPages, totalPages, currentPageNum === totalPages));

...
import { API_BASE_URL, getToken } from '../../ts/main.js';
...
function renderAdminUsers(users) {
    if (!userListContainer || !users)
        return;
    if (users.length === 0) { /* ... no users message ... */
        return;
    }
    const tableHtml = `
        <table>
...
async function handleUpdateUserRole(event) {
    ...
    if (newRole === 'seller')
        rolesToSet.push('seller');
    if (newRole === 'admin')
        rolesToSet.push('admin');

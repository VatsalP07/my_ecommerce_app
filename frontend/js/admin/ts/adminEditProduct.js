...
async function fetchProductForEdit(productId) {
    ...
    const token = getToken();
    if (!token)
        return; // Should be handled by updateAdminNavOnEditPage
    try {
...
function renderEditForm(product) {
    ...
    if (product.imageKeys && product.imageKeys.length > 0) {
        ...
        product.imageKeys.forEach((imgUrl) => {
...
async function handleUpdateProduct(event) {
    ...
    const newImageFilesInput = document.getElementById('newProductImages');
    ...
    try {
        ...
        if (successElement)
            successElement.textContent = 'Product updated successfully!';
        setTimeout(() => {
            if (successElement)
                successElement.textContent = '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (productId && getToken()) { // Check token before fetching
        ...
        if (formContainer)
            formContainer.innerHTML = '<p>No product ID specified for editing.</p>';
    }
});

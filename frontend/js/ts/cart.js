...
async function handleQuantityChange(event, type) {
    ...
    if (type === 'direct') {
        ...
        quantityInput = parentDiv?.querySelector('.item-quantity-input');
    }
    ...
    placeOrderTestBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (paymentSuccess) {
                ...
                if (checkoutMessageEl)
                    checkoutMessageEl.textContent = 'Simulated payment failed. Order created but payment pending.';

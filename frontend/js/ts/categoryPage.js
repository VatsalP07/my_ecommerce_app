...
async function fetchCategoryProducts(categoryName, page = 1) {
    if (!categoryProductListContainer || !categoryName)
        return;
    categoryProductListContainer.innerHTML = '<p class="col-span-full text-center text-gray-500 py-10">Loading products...</p>';
    if (categoryPaginationControlsContainer)
...
function renderCategoryPaginationControls(pagination) {
    ...
    if (currentPageNum > pageBuffer + 2 && totalPages > maxPagesToShowOverall)
        categoryPaginationControlsContainer.appendChild(createButton('...', 0, false, true, true));
    let startPage = Math.max(2, currentPageNum - pageBuffer);
    let endPage = Math.min(totalPages - 1, currentPageNum + pageBuffer);
    for (let i = startPage; i <= endPage; i++)
        categoryPaginationControlsContainer.appendChild(createButton(i, i, currentPageNum === i));
    if (currentPageNum < totalPages - pageBuffer - 1 && totalPages > maxPagesToShowOverall)
        categoryPaginationControlsContainer.appendChild(createButton('...', 0, false, true, true));
    if (totalPages > 1)
        categoryPaginationControlsContainer.appendChild(createButton(totalPages, totalPages, currentPageNum === totalPages));
...
function updateStockOnCategoryPage(productId, newStock) {
